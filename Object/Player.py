...
import time
class Player: 
    def __init__(self, x, y, screen, images, boards, music):
        self.x = x
        self.y = y
        self.direction = 0
    ...
    def check_collision_no_power_up(self, ghosts, setDecreaseLives): 
       if not self.power_up:
            ...
            for ghost in ghosts:
                ...
                if distance < DISTANCE_COLLISION and not ghost.dead:  
                    if self.lives > 0:
                        if (setDecreaseLives == True): 
                            self.lives -= 1
                        if (utils.info_record != None):
                            utils.info_record.showRecord()
                            utils.info_record.isShowRecord = True
                        self.music.musicPacmanDeath()
                        time.sleep(TIMER_PACMAN_DEATH)
                        ...
                        self.startup_counter = 0
                    break
    def eat_ghost(self, ghosts, setDecreaseLives):
        center_x, center_y = utils.getCenter(self.x, self.y, boards)
        if self.power_up:
            for ghost in ghosts:
                ghost1 = ["Đỏ", "Xanh", "Hồng", "Cam"]
                # print("info ghost: ", ghost1[ghost.id], self.power_up, ghost.dead, ghost.eaten, "2=========")
                distance = math.sqrt((center_x - ghost.center_x) ** 2 + (center_y - ghost.center_y) ** 2)
                # if distance < DISTANCE_COLLISION and not ghost.dead and not ghost.eaten:  
                if distance < DISTANCE_COLLISION and ghost.eaten and not ghost.dead:
                    ghost.eaten = False
                    self.score += SCORE_GHOST
                    ghost.dead = True
                    self.music.musicPacmanEatGhost()
                elif distance < DISTANCE_COLLISION and not ghost.eaten and not ghost.dead:
                    if self.lives > 0:
                        if (setDecreaseLives == True): 
                            self.lives -= 1
                        if (utils.info_record != None):
